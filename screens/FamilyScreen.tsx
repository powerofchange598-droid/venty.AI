...
const FamilyScreen: React.FC<FamilyScreenProps> = ({ user, family: initialFamily, onUpdateFamily, purchaseRequests }) => {
    ...
    const [newMonthlyContribution, setNewMonthlyContribution] = useState<number>(0);
    const [newDeadline, setNewDeadline] = useState<string>(getTomorrowISO());
    ...
    const handleAddGoal = () => {
        const goal: Goal = {
            ...
            name: newGoalName || 'New Goal',
            targetAmount: Number(newTargetAmount) || 0,
            currentAmount: 0,
            investedAmount: 0,
            deadline: newDeadline,
            monthlyContribution: Number(newMonthlyContribution) || 0,
        };
        onUpdateFamily({
            ...family,
            sharedGoals: [...family.sharedGoals, goal],
        });
        setShowAddGoal(false);
        setNewGoalName('');
        setNewTargetAmount(0);
        setNewMonthlyContribution(0);
        setNewDeadline(getTomorrowISO());
    };
            ...
            <div className="py-6 space-y-6">
                ...
                <div>
                    ...
                    {user.isFamilyHead && (
                        <div className="mb-4">
                            {!showAddGoal ? (
                                <VentyButton onClick={() => setShowAddGoal(true)} className="!w-auto !py-2 !px-3">
                                    <PlusIcon className="h-5 w-5 mr-2"/> {t('Add Goal')}
                                </VentyButton>
                            ) : (
                                <Card>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input type="text" value={newGoalName} onChange={e => setNewGoalName(e.target.value)} className="w-full p-2 mt-1" />
                                        </div>
                                        <div>
                                            <label className="text-sm font-medium">Target Amount</label>
                                            <input type="number" value={newTargetAmount} onChange={e => setNewTargetAmount(Number(e.target.value))} className="w-full p-2 mt-1" />
                                        </div>
                                        <div>
                                            <label className="text-sm font-medium">Monthly Contribution</label>
                                            <input type="number" value={newMonthlyContribution} onChange={e => setNewMonthlyContribution(Number(e.target.value))} className="w-full p-2 mt-1" />
                                        </div>
                                        <div>
                                            <label className="text-sm font-medium">Deadline</label>
                                            <input type="date" value={newDeadline} onChange={e => setNewDeadline(e.target.value)} className="w-full p-2 mt-1" />
                                        </div>
                                    </div>
                                    <div className="flex justify-end space-x-2 mt-4">
                                        <VentyButton onClick={() => setShowAddGoal(false)} variant="secondary" className="!w-auto !py-1 !px-3 !text-sm">Cancel</VentyButton>
                                        <VentyButton onClick={handleAddGoal} className="!w-auto !py-1 !px-3 !text-sm">Save</VentyButton>
                                    </div>
                                </Card>
                            )}
                        </div>
